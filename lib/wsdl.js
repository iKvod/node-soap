...
function splitType(typeName) {
    var i = (typeName != null) ? typeName.indexOf(':') : -1;
    return i < 0 ? {namespace:null,name:typeName} : {alias:typeName.substring(0, i), type:typeName.substring(i+1)};
}
...
var Element = function(nsName, attrs) {
    ...
    if (attrs['type']) {
        this.elementType = splitType(attrs['type']);
    }
}
...
Element.prototype.endElement = function(stack, nsName) {
    if (this.nsName === nsName) {
        ...
        if (this.elementType && this.elementType.alias) {
            this.elementType.xmlns = stack[0].xmlns[this.elementType.alias];
        }
        stack.pop();
    }
}
...
var PolicyElement = Element.createSubClass();
var WsdlImportElement = Element.createSubClass();
...
var ElementTypeMap = {
    ...
    definitions: [DefinitionsElement, 'types message portType binding service Policy import'],
    import: [WsdlImportElement, ''],
...
WsdlImportElement.prototype.init = function() {
}
...
DefinitionsElement.prototype.addChild = function(child) {
    ...
    else if (child instanceof PolicyElement) {
        self.policy = child;
    }
    else if (child instanceof WsdlImportElement) {
        var location = child.$schemaLocation || child.$location;
        if (location) {
            self.wsdlImports.push({
                namespace: child.$namespace || child.$targetNamespace || this.$targetNamespace,
                location: location
            });
        }
...
var WSDL = function(definition, uri, options) {
    ...
    this.excludedImports = this.options.excludedImports || [];
    if (this.excludedImports.indexOf(uri) == -1) {
        this.excludedImports.push(uri);
    }
...
WSDL.prototype._mergeDefinitions = function(wsdl) {
    var wsdlDefinitions = wsdl.definitions,
        self = this,
        propertiesToMerge = ['portTypes', 'bindings', 'messages', 'services', 'schemas'];

    function mergeProperty(propertyName) {
        for (var childPropertyName in wsdlDefinitions[propertyName]) {
            if (wsdlDefinitions[propertyName].hasOwnProperty(childPropertyName)) {
                if (!self.definitions[propertyName][childPropertyName]) {
                    self.definitions[propertyName][childPropertyName] = wsdlDefinitions[propertyName][childPropertyName];
                }
            }
        }
    }

    propertiesToMerge.forEach(function(propertyToMerge) {
        mergeProperty(propertyToMerge);
    });

}

WSDL.prototype._mergeExcludedImports = function(newExcludedImports) {
    var self = this;
    newExcludedImports.forEach(function(excludedImport) {
        if (self.excludedImports.indexOf(excludedImport) == -1) {
            self.excludedImports.push(excludedImport);
        }
    });
}
...
WSDL.prototype._processNextInclude = function(includes, callback) {
    ...
    if (self.excludedImports.indexOf(includePath) != -1) {
        self._processNextInclude(includes, function(err) {
            callback(err);
        })
    } else {
        open_wsdl(includePath, {excludedImports: self.excludedImports}, function(err, wsdl) {
            if (err) {
                return callback(err);
            }
            if (wsdl.services) {
                self._mergeDefinitions(wsdl);
            } else {
                self.definitions.schemas[include.namespace || wsdl.definitions.$targetNamespace] = wsdl.definitions;
            }

            self._mergeExcludedImports(wsdl.excludedImports);
...
WSDL.prototype.processIncludes = function(callback) {
    var schemas = this.definitions.schemas,
        includes = [],
        wsdlImports = this.definitions.wsdlImports;

    for (var ns in schemas) {
        var schema = schemas[ns];
        includes = includes.concat(schema.includes || [])
    }
    includes = includes.concat(wsdlImports || []);
...
WSDL.prototype.objectToDocumentXML = function(name, params, ns, xmlns, schemaObject) {
    var args = {};
    args[name] = params;
    return this.objectToXML(args, null, ns, xmlns, true, schemaObject);
}

WSDL.prototype._getReducedNameSpaceValue = function(ns, xmlns) {
    return xmlns ? findKey(this.xmlnsMappings, xmlns) : ns;
}
...
function findChildOfElement(schemaObject, childName) {
    if (schemaObject) {
        var firstChild = schemaObject.children[0];
        if (firstChild && firstChild.name == 'complexType') {
            var complexTypeFirstChild = firstChild.children[0];
            if (complexTypeFirstChild.name == 'sequence') {
                for (var i in complexTypeFirstChild.children) {
                    if (complexTypeFirstChild.children[i].$name == childName) {
                        return complexTypeFirstChild.children[i];
                    }
                }
            }
        }
...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, first, schemaObject) {
    namespace = this._getReducedNameSpaceValue(namespace, xmlns);
    var self = this,
        parts = [],
    ...
    if (Array.isArray(obj)) {
        for (var i=0, item; item=obj[i]; i++) {
            ...
            parts.push(self.objectToXML(item, name, namespace, xmlns, false, schemaObject));
        }
    }
        ...
        for (var name in obj) {
            var child = obj[name];
            parts.push(['<',ns,name,xmlnsAttrib,'>'].join(''));
            if (!first && schemaObject) {
                var childSchema = findChildOfElement(schemaObject, name);
                if (childSchema) {
                    var childXmlns = childSchema.elementType.xmlns;
                    var childAlias = this._getReducedNameSpaceValue(undefined, childXmlns);
                    parts.push(self.objectToXML(child, name, childAlias, childXmlns, false, childSchema));
                } else {
                    parts.push(self.objectToXML(child, name, namespace, xmlns, false, schemaObject));
                }
            } else {
                parts.push(self.objectToXML(child, name, namespace, xmlns, false, schemaObject));
            }
...
WSDL.prototype._mapSchemasToXmlns = function() {
    var xmlnsAliases = {};
    var str = '';
    var aliasPrefix = 'ns';
    var i = 0;
    for (var ns in this.definitions.schemas) {
        switch(ns) {
            case "http://xml.apache.org/xml-soap" : // apachesoap
            case "http://schemas.xmlsoap.org/wsdl/" : // wsdl
            case "http://schemas.xmlsoap.org/wsdl/soap/" : // wsdlsoap
        ...
        if (~ns.indexOf('http://www.w3.org/')) continue;
        if (~ns.indexOf('http://xml.apache.org/')) continue;
        var aliasValue = findKey(this.definitions.xmlns, ns) || aliasPrefix + i++;
        xmlnsAliases[aliasValue] = ns;
        str += ' xmlns:' + aliasValue + '="' + ns + '"';
    }
    this.xmlnsMappings = xmlnsAliases;
    this.xmlnsInEnvelope = str;
}
