...
var WSDL = function(definition, uri, options) {
    ...
    if (!this.options.excludedWsdls) {
        this.options.excludedWsdls = [];
    }
    if (this.options.excludedWsdls.indexOf(uri) == -1) {
        this.options.excludedWsdls.push(uri);
    }
...
WSDL.prototype._mergeDefinitions = function(wsdl) {
    ...
    self.options.excludedWsdls = self.options.excludedWsdls.concat(wsdl.options.excludedWsdls || []);
...
WSDL.prototype._processNextInclude = function(includes, callback) {
    ...
    if (this.options.excludedWsdls.indexOf(includePath) != -1) {
        self._processNextInclude(includes, function(err) {
            callback(err);
        })
    } else {
        open_wsdl(includePath, {excludedWsdls: self.options.excludedWsdls}, function(err, wsdl) {
            if (err) {
                return callback(err);
            }
            if (wsdl.services) {
